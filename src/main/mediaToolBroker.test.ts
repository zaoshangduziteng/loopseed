...
describe('media tool broker', () => {
  it('publishes bounded top-level asset and generation function tools', () => {
    ...
    expect(MEDIA_DYNAMIC_TOOLS.map((tool) => tool.name)).toEqual([
      'loopseed_asset_list',
      'loopseed_asset_register',
      'loopseed_audio_synthesize',
      'loopseed_image_generate',
      'loopseed_audio_generate',
      'loopseed_model3d_generate',
    ]);
    ...
    const audioTool = MEDIA_DYNAMIC_TOOLS.find((tool) => tool.name === 'loopseed_audio_generate');
  ...
  it('lists only compact public asset fields and caps protocol output', async () => {
    ...
    broker.handle(toolRequest(3, 'loopseed_asset_list', { kind: 'image', limit: 100 }));
    await vi.waitFor(() => expect(responses).toHaveLength(1));
  ...
  it('creates and registers a short procedural WAV through a dynamic call', async () => {
    const root = await mkdtemp(join(tmpdir(), 'loopseed-media-broker-'));
    roots.push(root);
    ...
    const request = toolRequest(4, 'loopseed_audio_synthesize', {
      name: 'game_pickup',
      preset: 'pickup',
      durationSeconds: 0.1,
      seed: 99,
  ...
  it('rejects traversal and invalid names without exposing absolute paths', async () => {
    const root = await mkdtemp(join(tmpdir(), 'loopseed-media-safe-'));
    roots.push(root);
    const responses: Array<{ id: string | number; result: unknown }> = [];
    const broker = brokerWith({
      responses,
    ...
    broker.handle(toolRequest(5, 'loopseed_asset_register', {
      relativePath: 'public/assets/../../outside.wav',
    }));
    broker.handle(toolRequest(6, 'loopseed_audio_synthesize', {
      name: '../outside',
      preset: 'hit',
    }));
  ...
  it('returns an explicit Codex ImageGen fallback without placing binary data in JSON-RPC', async () => {
    ...
    broker.handle(toolRequest(7, 'loopseed_image_generate', {
      name: 'fox_run',
      prompt: 'Four-frame fox run sprite sheet',
      width: 1024,
      height: 1024,
  ...
  it('routes audio and 3D API generation and only returns compact public asset fields', async () => {
    ...
    broker.handle(toolRequest(8, 'loopseed_model3d_generate', {
      name: 'player_ship',
  ...
  it('passes explicit MiniMax audio purpose, instrumental mode, and lyrics as provider options', async () => {
    ...
    broker.handle(toolRequest(10, 'loopseed_audio_generate', {
      name: 'title_music',
      prompt: 'Tense survival-horror title music',
      purpose: 'music',
      instrumental: false,
  ...
  it('rejects unsupported audio purposes before invoking a provider', async () => {
    ...
    broker.handle(toolRequest(11, 'loopseed_audio_generate', {
      name: 'rifle_shot',
  ...
  it('rejects a missing audio purpose before invoking a provider', async () => {
    ...
    broker.handle(toolRequest(111, 'loopseed_audio_generate', {
      name: 'mystery_audio',
  ...
  it('returns an actionable procedural fallback for generic SFX that MiniMax does not support', async () => {
    ...
    broker.handle(toolRequest(12, 'loopseed_audio_generate', {
      name: 'rifle_shot',
      prompt: 'Close-range rifle shot with concrete echo',
      purpose: 'sfx',
    }));
  ...
  it('rejects incomplete image dimensions before calling a provider', async () => {
    ...
    broker.handle(toolRequest(9, 'loopseed_image_generate', {
      name: 'hero',
      prompt: 'Hero',
      width: 1024,
    }));
  ...
  it('surfaces only trusted redacted media failures and keeps unexpected errors opaque', async () => {
    ...
    safeBroker.handle(toolRequest(91, 'loopseed_audio_generate', {
      name: 'safe_error',
    ...
    opaqueBroker.handle(toolRequest(92, 'loopseed_audio_generate', {
      name: 'opaque_error',
      prompt: 'Minimal music test',
      purpose: 'music',
    }));
  ...
  it('passes a real MiniMax business failure through service and broker without provider secrets', async () => {
    ...
    broker.handle(toolRequest(93, 'loopseed_audio_generate', {
